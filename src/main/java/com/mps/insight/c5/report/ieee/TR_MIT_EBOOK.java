...
public class TR_MIT_EBOOK {
	...
	public TR_MIT_EBOOK(Counter5DTO dto, String previewType, RequestMetaData rmd) {
		this.dto = dto;
	...
	public void run() {
		includeMonth();
		generatMITEBOOKReport();
	}

	public void generatMITEBOOKReport() {
		StringBuilder stb = new StringBuilder();
		try {
			stb.append("SELECT ");
			stb.append(" " + InsightConstant.MIT_EBOOK_MASTER_TITLE + ",");
			...
			stb.append(" and " + InsightConstant.MIT_EBOOK_WHERE_CONDITION);

			stb.append(" AND" + totalMonth + ">0 ");
			stb.append("GROUP BY " + InsightConstant.MIT_EBOOK_MASTER_TITLE);
			stb.append(" ORDER BY `Item` ");
			...
			rmd.exception("MIT EBOOK : generat_MIT_EBOOK_Report : Unable to create query " + e.toString());
		}
	...
	public void includeMonth() {
		try {
			...
			rmd.exception("MIT EBOOK : unable to add month in query" + e.toString());
		}
	}

...
public class MitEbookChapterRequestByMonth {

	private Counter5DTO dto = null;
	private Counter5ReportsDao c5dao = null;
	private String coloumTitle = " mitebook.* FROM (SELECT '' AS `Book ID`, '' AS `Book Title`, '' AS ISBN, 'Total for all Chapter Requests' AS `Chapter Title`, '' AS `Chapter ID`,'' AS Metric_Type,";
	private String unColoumTitle = " PARENT_ID AS `Book ID`, PARENT_TITLE AS `Book Title`, ISBN AS ISBN, ITEM AS `Chapter Title`, ITEM_ID AS `Chapter ID`,metric_type,";

	public MitEbookChapterRequestByMonth(Counter5DTO c5Dto, RequestMetaData rmd) {
	...
	private String getQuery(String download) {
		...
		String tableName = " `c5_mit_ebook_chapter_by_month` ";
		try {
			...
			total = dmoncreate.createTotalMonthQueryC5(tomonth, toyear, frommonth, fromyear, "Reporting_Period_Total");
			...
			query.append("select " + coloumTitle + dynamicmonth1 + "" + dynamicmonth + " AS `Reporting_Period_Total`")
					...
					.append(" UNION ALL select " + unColoumTitle + dynamicSumMonth + "," + total + " AS `Reporting_Period_Total`")
					.append(" from " + tableName).append(" Where " + total + ">0 ) mitebook ")
					.append(" ORDER BY mitebook.`Reporting_Period_Total` DESC, mitebook.`Chapter Title` = 'Total for all Chapter Requests' DESC, mitebook.`Chapter Title` ASC ");

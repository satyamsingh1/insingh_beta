...
public class TR_STD {
	...
	public TR_STD(Counter5DTO dto,String previewType,RequestMetaData rmd){
		this.dto=dto;
	...
	public void run(){
		...
		generateStandardQuery();
		
	}
	
	private String  generateStandardQuery(){
		StringBuilder query = new StringBuilder();
		try {
			query .append("SELECT "); 
			query .append(" "+InsightConstant.STANDARD_MASTER_TITLE); //`Parent_Title` AS title ,`Publisher`,.....`Metric_Type`
			...
			query.append(" and "+InsightConstant.STANDARD_WHERE_CONDITION); //`Metric_Type` IN ('Total_Item_Requests','Unique_Item_Requests') AND
			query.append(" and "+totalMonth+" > 0 ");
			query.append(" "+InsightConstant.STANDARD_GROUPBY); //GROUP BY `Parent_Title` ,`Publisher`,`Publisher_ID`............
			...
			rmd.exception("generateStandardQuery : unable to create Query : "+e.toString());
		}

...
/*	private String tableName = "master_report";*/
	...
	public TR_WILEY_EBOOK(Counter5DTO dto,String previewType,RequestMetaData rmd){
		this.dto=dto;
	...
	public void run(){
		...
		generateTR_WIBCRMIQuery();
		
	}
	
	private String  generateTR_WIBCRMIQuery(){
		StringBuilder query = new StringBuilder();
		try {
			query .append("SELECT "); 
			query .append(" "+InsightConstant.TR_WIBCRMI_MASTER_TITLE); //`Parent_Title` AS title ,`Publisher`,.....`Metric_Type`
			...
			query.append(" and "+InsightConstant.TR_WIBCRMI_WHERE_CONDITION); //`Metric_Type` IN ('Total_Item_Requests','Unique_Item_Requests') AND
			query.append(" and "+totalMonth+" > 0 ");
			query.append(" "+InsightConstant.TR_WIBCRMI_GROUPBY); //GROUP BY `Parent_Title` ,`Publisher`,`Publisher_ID`............
			...
			rmd.exception("generateTR_WIBCRMIQuery : unable to create Query : "+e.toString());
		}

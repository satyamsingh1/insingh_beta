...
public class ArticleRequestByMonth {

	private Counter5DTO dto = null;
	private Counter5ReportsDao c5dao = null;
	private String coloumTitle = "TITLE_TYPE AS ` Total for all Types`,METRIC_TYPE AS `Metric_Type`, ";

	public ArticleRequestByMonth(Counter5DTO c5Dto, RequestMetaData rmd) {
	...
	private String getQuery(String download) {
		...
		String tableName = "c5_article_request_by_month"; //from config/hardcode/tabvle/xtz/etc
		try {
			...
			.append(" from "+tableName + " GROUP BY TITLE_TYPE ,METRIC_TYPE ORDER BY TITLE_TYPE");

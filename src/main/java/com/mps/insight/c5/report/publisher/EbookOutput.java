...
public class EbookOutput {
	...
	private String coloumTitle = "`Institution_ID` AS 'Account ID',`Institution_Name` AS 'Institution or Customer ID',`Institution_Type` AS 'Type',`Title` AS 'eBook ID',`Metric_Type`,";
	private String groupTitle = " GROUP BY `Institution_ID`,`Institution_Name`,`Institution_Type`,`Title`,`Metric_Type` ";
	public EbookOutput(Counter5DTO c5Dto, RequestMetaData rmd) {
	...
	private String getQuery(String download) {
		...
		String tableName = "c5_ebook_output_and_details"; 
		try {
			...
			.append(" from " + tableName+" WHERE  Title='Total for all Titles' AND institution_id NOT IN ('-','',' ') "+groupTitle+"  ORDER BY Institution_ID");

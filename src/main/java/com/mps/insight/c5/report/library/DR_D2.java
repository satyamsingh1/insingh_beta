...
public class DR_D2 {
	...
	public DR_D2(Counter5DTO dto,String previewType,RequestMetaData rmd)
	{
		this.dto=dto;
	...
	public void generatDR2Report()
	{
		...
		try
		{
			...
			stb .append("CONCAT('"+dto.getPublisher()+"',':',`Proprietary_ID`) AS Proprietary_ID, "); 
			stb .append("`Metric_Type`, ");
			...
			stb.append(" and "+InsightConstant.DR_D2_WHERE_CONDITION);
			stb.append(" AND"+ totalMonth+">0 ");
			stb.append("GROUP BY "+InsightConstant.DR_D2_MASTER_TITLE);
		...
		{
			rmd.exception("DR_D2 : generatDR_D1Report : Unable to create query "+e.toString());
		}
	...
	public void includeMonth()
	{
		...
		{
			rmd.exception("DR_D2 : unable to add month in query"+e.toString());
		}
	}

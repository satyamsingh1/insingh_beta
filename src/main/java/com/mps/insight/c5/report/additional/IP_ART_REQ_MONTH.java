...
public class IP_ART_REQ_MONTH {
	...
	public IP_ART_REQ_MONTH(Counter5DTO dto,String previewType,RequestMetaData rmd){
		this.dto=dto;
	...
	public void run(){
		...
		generateIP_ART_REQ_MONTHQuery();
		
	}
	
	private String  generateIP_ART_REQ_MONTHQuery(){
		StringBuilder query = new StringBuilder();
		try {
			//query.append(" SELECT 'IP_ART_REQ_MONTH is in progress' AS Message FROM DUAL ");
			query .append("SELECT "); 
			query .append(" "+InsightConstant.IP_ART_REQ_MONTH_MASTER_TITLE); //`Parent_Title` AS title ,`Publisher`,.....`Metric_Type`
			...
			query.append(" and "+InsightConstant.IP_ART_REQ_MONTH_WHERE_CONDITION); //`Metric_Type` IN ('Total_Item_Requests','Unique_Item_Requests') AND
			query.append(" and "+totalMonth+" > 0 ");
			query.append(" "+InsightConstant.IP_ART_REQ_MONTH_GROUPBY); //GROUP BY `Parent_Title` ,`Publisher`,`Publisher_ID`............
			...
			rmd.exception("generateIP_ART_REQ_MONTHQuery : unable to create Query : "+e.toString());
		}

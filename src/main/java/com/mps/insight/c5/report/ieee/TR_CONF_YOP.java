...
public class TR_CONF_YOP {
	...
	public TR_CONF_YOP(Counter5DTO dto,String previewType,RequestMetaData rmd){
		this.dto=dto;
	...
	public void run(){
		...
		generateTR_CONF4Query();
		
	}
	
	private String  generateTR_CONF4Query(){
		StringBuilder query = new StringBuilder();
		try {
			query .append("SELECT "); 
			query .append(" "+InsightConstant.TR_CONF_YOP_MASTER_TITLE); //`Parent_Title` AS title ,`Publisher`,.....`Metric_Type`
			...
			query.append(" and "+InsightConstant.TR_CONF_YOP_WHERE_CONDITION); //`Metric_Type` IN ('Total_Item_Requests','Unique_Item_Requests') AND
			query.append(" and "+totalMonth+" > 0 ");
			query.append(" "+InsightConstant.TR_CONF_YOP_GROUPBY); //GROUP BY `Parent_Title` ,`Publisher`,`Publisher_ID`............
			...
			rmd.exception("generateTR_CONF4Query : unable to create Query : "+e.toString());
		}

...
public class TR_LIC_DEN_MONTH {
	...
	public TR_LIC_DEN_MONTH(Counter5DTO dto,String previewType,RequestMetaData rmd){
		this.dto=dto;
	...
	private String  generateTR_LIC_DEN_MONTHQuery(){
		...
		try {
			query .append("SELECT "); 
			query .append(" "+InsightConstant.TR_LIC_DEN_MONTH_MASTER_TITLE); //`Parent_Title` AS title ,`Publisher`,.....`Metric_Type`
			...
			query.append(" and "+InsightConstant.TR_LIC_DEN_MONTH_WHERE_CONDITION); //`Metric_Type` IN ('Total_Item_Requests','Unique_Item_Requests') AND
			query.append(" and "+totalMonth+" > 0 ");
			query.append(" "+InsightConstant.TR_LIC_DEN_MONTH_GROUPBY); //GROUP BY `Parent_Title` ,`Publisher`,`Publisher_ID`............

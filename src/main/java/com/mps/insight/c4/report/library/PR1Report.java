...
public class PR1Report {

	DynamicMonthCreater dmc = new DynamicMonthCreater();
	private String tableName = "PlatformReport1";
	...
	private String queryHeader ="'Total for all Platform' AS Platform,'' AS `Publisher`,'' AS `UserActivity`";
	
	public PR1Report(String accountCode, String report, String from, String to) {
	...
	public void generatPR1Report() {
		...
		try {
			...
			stb.append("SELECT Platform,`Publisher`,`UserActivity`, ");
			stb.append("("+getMonthPlus(montList)+") AS `Reporting Period Total`, ");
			stb.append(montList+" ");
			stb.append("FROM "+tableName+" WHERE institution='"+accountCode+"'");
			
		} catch (Exception e) {
			MyLogger.error("PR1Report : generatDB1Report : Unable to create query " + e.toString());
		}
	...
	private void includeMonth() {
		try {
			...
			montList = dmc.getMonthListNew("PR1",  from, to);
			
		} catch (Exception e) {
			MyLogger.error("PR1Report : unable to add month in query" + e.toString());
		}
	}
